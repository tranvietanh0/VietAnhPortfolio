...
const ProjectCard = ({ project, index }) => {
  ...
  return (
    <motion.div
      className="group bg-dark-secondary rounded-xl overflow-hidden border border-white/5 hover:border-accent/30 transition-all duration-300 card-hover"
    ...
    >
      ...
      <div className="relative h-48 overflow-hidden bg-dark-tertiary">
        ...
        {/* Overlay */}
        <div className="absolute inset-0 bg-gradient-to-t from-dark-secondary via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
      ...
      <div className="p-6">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {project.tags.map((tag, idx) => (
            <span
              key={idx}
              className="px-2 py-1 text-xs font-medium bg-dark-tertiary text-text-secondary rounded-md"
            >
              {tag}
            </span>
          ))}
